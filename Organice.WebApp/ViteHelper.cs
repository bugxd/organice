    ...
    public static class ViteHelper
    {
        ...
        public static void UseViteDevelopmentServer(this ISpaBuilder spa, int? port = null, string sourcePath = null)
        {
            ...
            if (!CheckPortInUsed(spa.Options.DevServerPort))
            {
                ...
                // export dev cert
                var spaFolder = System.IO.Path.Combine(webHostEnvironment.ContentRootPath, spa.Options.SourcePath);
                ...
                var devCert = System.IO.Path.Combine(spaFolder, "devcert.pfx");
                var serverOptionFile = System.IO.Path.Combine(spaFolder, $"serverOption{new FileInfo(viteConfigPath).Extension}");
        ...
        private static string BuildServerOption(string certfile, string pass)
        {
            ...
            sb.AppendLine($"https: {{ pfx: '{System.IO.Path.GetFileName(certfile)}', passphrase: '{pass}' }}");
            sb.AppendLine("}");
            sb.AppendLine();
        ...
        private static void EnsureNodeModuleAlreadyInstalled(string sourcePath)
        {
            ...
            if (!Directory.Exists(System.IO.Path.Combine(sourcePath, "node_modules")))
            {
